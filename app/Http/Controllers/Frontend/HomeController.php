...
class HomeController extends Controller
{
    ...
    public  function index()
    {
        if (file_exists(storage_path('installed'))) {
            ...
            $data['products'] = $all_products->limit(allsetting('home_products_page'))->get();
            // dd($data['products']);
            $data['new_arrivals'] = Product::with('category')->where('New_Arrival', ACTIVE)->limit(allsetting('home_products_page'))->get();
            $data['best_sellings'] = Product::with('category')->where('Best_Selling', ACTIVE)->limit(allsetting('home_products_page'))->get();
            // $data['on_sales'] = $all_products->where('On_Sale', ACTIVE)->limit(allsetting('home_products_page'))->get();
            $data['featured_products'] =Product::with('category')->where('Featured_Product', ACTIVE)->limit(allsetting('home_products_page'))->get();
